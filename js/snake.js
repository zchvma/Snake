...
function wouldTrapSnake(gameState, newWalls) {
    ...
    while (queue.length > 0) {
        ...
        const directions = [{row: current.row - 1, col: current.col}, // вверх
        ...
        for (const next of directions) {
            ...
            if (!visited.has(key) && (nextRow < 0 || nextRow >= MAX_GRID.height || nextCol < 0 || nextCol >= MAX_GRID.width || grid[nextRow][nextCol] === 0)) {
                visited.add(key)
